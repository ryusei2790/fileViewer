...
| **Markdown内画像表示** | Markdown内の相対パス画像参照（`![](./images/photo.png)`）を自動解決して描画 |
...
graph TB
    ...
    Rust -->|fs::read / canonicalize| FS[(ローカルファイルシステム)]
```
...
| Markdown | `.md`, `.markdown` | marked + highlight.js による変換表示。相対パスの画像参照を自動解決しインライン描画 | テキスト（UTF-8） |
